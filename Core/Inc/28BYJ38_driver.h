...
/* Start ruchu (nieblokujący). Jeśli steps<=0 — nic nie robi. */
void StepMotor_Run(int32_t steps, int8_t direction, uint16_t interval_us);

/* Zadanie „w tle”: wykona co najwyżej jeden półkrok, gdy minie interwał. */
void StepMotor_Handler(void);

/* Drobne pomocnicze (opcjonalnie wygodne w main.c): */
